...
pub trait Backend: Sized {
    ...
    type Tensor<'a, T: DType, S: Store, D: Dim>: Tensor<'a, T, Self, S, D>
    where
        Self: SupportsDType<T>;
}

pub trait SupportsDType<T: DType>: Backend {
    type _Tensor<'a, S: Store, D: Dim>: Tensor<'a, T, Self, S, D>
    where
        Self: Sized;
}
...
pub trait ModelLoader<B: Backend>: Sized {
    ...
    fn load_tensor<'a, T: DType, D: Dim>(
        &'a self,
        name: &str,
    ) -> LoadResult<B::Tensor<'a, T, LoadStore, D>>
    where
        B: SupportsDType<T>;
}
    ...
    Sized + Debug + Clone
{
    ...
    fn to_dtype<'b, U: DType>(&self) -> B::Tensor<'b, U, OwnedStore, D>
    where
        B: SupportsDType<U>;
    ...
    fn map_axis(
        &self,
        axis: usize,
        f: impl Fn(RefTensor1<B, T>) -> T,
    ) -> B::Tensor<'a, T, OwnedStore, D::Smaller>;
    fn map_axis_threaded(
        &self,
        axis: usize,
        f: impl Fn(RefTensor1<B, T>) -> T + Send + Sync,
    ) -> B::Tensor<'a, T, OwnedStore, D::Smaller>;
