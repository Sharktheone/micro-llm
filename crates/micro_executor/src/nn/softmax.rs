use micro_backend::{
    Backend, DType, RefTensor1, RefTensor2, RefTensor3, SupportsDType, Tensor, Tensor1, Tensor2,
    Tensor3,
};
use num_traits::Float;

pub fn softmax1<'a, B: Backend + SupportsDType<T>, T: DType>(
    input: &RefTensor1<B, T>,
) -> Tensor1<'a, B, T> {
...
fn softmax2<'a, B: Backend + SupportsDType<T>, T: DType>(
    input: &RefTensor2<B, T>,
    axis: usize,
) -> Tensor2<'a, B, T> {
...
fn softmax2_inplace<B: Backend + SupportsDType<T>, T: DType>(
    input: &mut Tensor2<B, T>,
    axis: usize,
) {
    let max = input.map_axis_threaded(axis, |col| {
        col.data().iter().cloned().fold(T::min_value(), T::max)
...
fn softmax3<'a, B: Backend + SupportsDType<T>, T: DType>(
...
fn softmax3_inplace<B: Backend + SupportsDType<T>, T: DType>(
    input: &mut Tensor3<B, T>,
    axis: usize,
) {
    let max = input.map_axis_threaded(axis, |col| {
        col.data().iter().cloned().fold(T::min_value(), T::max)
